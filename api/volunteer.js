...
export default async function handler(req, res) {
  ...
  if (!name || !email || !phone || !interest || !availability) {
    return res.status(400).json({
      error: 'Missing required fields: name, email, phone, interest, and availability are required.',
    });
  }
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      to: ['volunteer@sithelonpo.co.za'],
      replyTo: email,
      subject: `New Volunteer Application: ${safeName}`,
      html: `
        ...
        <table style="border-collapse:collapse;width:100%;max-width:600px;">
          ...
          <tr><td style="padding:8px;font-weight:bold;border-bottom:1px solid #eee;">Area of Interest</td><td style="padding:8px;border-bottom:1px solid #eee;">${safeInterest}</td></tr>
          <tr><td style="padding:8px;font-weight:bold;border-bottom:1px solid #eee;">Availability</td><td style="padding:8px;border-bottom:1px solid #eee;">${safeAvailability}</td></tr>
          <tr><td style="padding:8px;font-weight:bold;vertical-align:top;">Additional Info</td><td style="padding:8px;">${safeMessage}</td></tr>
        </table>
      `,
    });
